...
function Flatpickr(e, t) {
    ...
    function r() {
        ...
        }), K.isMobile || (document.addEventListener("keydown", y), window.addEventListener("resize", B(C, 300)), document.addEventListener("click", h), document.addEventListener("blur", h), K.config.clickOpens && (K.altInput || K.input).addEventListener("focus", w), K.config.noCalendar || (K.prevMonthNav.addEventListener("click", function () {
            return p(-1)
        }), K.nextMonthNav.addEventListener("click", function () {
            return p(1)
        }), K.currentYearElement.addEventListener("wheel", U), K.currentYearElement.addEventListener("focus", function () {
            K.currentYearElement.select()
        }), K.currentYearElement.addEventListener("input", function (e) {
            4 === e.target.value.length && K.currentYearElement.blur(), K.currentYear = parseInt(e.target.value, 10) || K.currentYear, K.redraw()
    ...
    function i(e) {
        e = e ? E(e) : K.selectedDateObj || K.config.defaultDate || K.config.minDate || K.now, K.currentYear = e.getFullYear(), K.currentMonth = e.getMonth(), K.redraw()
    }

    function o() {
        var e = document.createDocumentFragment();
        K.calendarContainer = W("div", "flatpickr-calendar"), K.config.noCalendar || (e.appendChild(c()), K.config.weekNumbers && e.appendChild(d()), K.rContainer = W("div", "flatpickr-rContainer"), K.rContainer.appendChild(s()), K.rContainer.appendChild(l()), e.appendChild(K.rContainer)), K.config.enableTime && e.appendChild(u()), K.calendarContainer.appendChild(e), K.config.inline || K.config.static ? (K.calendarContainer.classList.add(K.config.inline ? "inline" : "static"), I(), K.element.parentNode.appendChild(K.calendarContainer)) : document.body.appendChild(K.calendarContainer)
    }
    ...
    function l() {
        ...
        var e = (new Date(K.currentYear, K.currentMonth, 1).getDay() - Flatpickr.l10n.firstDayOfWeek + 7) % 7,
            t = K.utils.getDaysinMonth(), n = K.utils.getDaysinMonth((K.currentMonth - 1 + 12) % 12),
            a = document.createDocumentFragment(), r = n + 1 - e, i = void 0, o = void 0;
        for (K.config.weekNumbers && (K.weekNumbers.innerHTML = ""), K.days.innerHTML = ""; r <= n; r++) {
            var l = new Date(K.currentYear, K.currentMonth - 1, r, 0, 0, 0, 0, 0), c = k(l),
                u = W("span", "flatpickr-day prevMonthDay" + (c ? "" : " disabled"), r);
            c && (u.tabIndex = 0), a.appendChild(u)
        }
        for (r = 1; r <= t; r++) {
            i = new Date(K.currentYear, K.currentMonth, r, 0, 0, 0, 0, 0), K.config.weekNumbers && r % 7 === 1 && K.weekNumbers.insertAdjacentHTML("beforeend", "<span class='disabled flatpickr-day'>" + K.getWeek(i) + "</span>"), o = !k(i);
            var s = W("span", o ? "flatpickr-day disabled" : "flatpickr-day", r);
            o || (s.tabIndex = 0, J(i, new Date) && s.classList.add("today"), K.selectedDateObj && J(i, K.selectedDateObj) && (s.classList.add("selected"), K.selectedDateElem = s)), a.appendChild(s)
        }
        for (var d = t + 1; d <= 42 - e; d++) {
            var p = new Date(K.currentYear, K.currentMonth + 1, d % t, 0, 0, 0, 0, 0), f = k(p),
                m = W("span", "flatpickr-day nextMonthDay" + (f ? "" : " disabled"), d % t);
            K.config.weekNumbers && d % 7 === 1 && K.weekNumbers.insertAdjacentHTML("beforeend", "<span class='disabled flatpickr-day'>" + K.getWeek(p) + "</span>"), f && (m.tabIndex = 0), a.appendChild(m)
        }
    ...
    function m() {
        K.isOpen = !1, K.calendarContainer.classList.remove("open"), (K.altInput || K.input).classList.remove("active"), _("Close")
    }

    function g() {
        K.calendarContainer.parentNode.removeChild(K.calendarContainer), K.input.value = "", K.altInput && (K.input.type = "text", K.altInput.parentNode.removeChild(K.altInput)), document.removeEventListener("keydown", y), window.removeEventListener("resize", C), document.removeEventListener("click", h), document.removeEventListener("blur", h), delete K.input._flatpickr
    }

    function h(e) {
        var t = K.calendarContainer.contains(e.target), n = K.element.contains(e.target) || e.target === K.altInput;
    ...
    function M() {
        ...
        }), !K.config.dateFormat && K.config.enableTime && (K.config.dateFormat = Flatpickr.defaultConfig.dateFormat, K.config.noCalendar ? (K.config.dateFormat = "H:i" + (K.config.enableSeconds ? ":S" : ""), K.config.altFormat = "h:i" + (K.config.enableSeconds ? ":S K" : " K")) : (K.config.dateFormat += " H:i" + (K.config.enableSeconds ? ":S" : ""), K.config.altFormat = "h:i" + (K.config.enableSeconds ? ":S" : "") + " K")), Object.keys(Flatpickr.defaultConfig).forEach(function (e) {
            return K.config[e] = "undefined" != typeof K.config[e] ? K.config[e] : Flatpickr.defaultConfig[e]
        })
    }

    function E(e) {
        var t = !(arguments.length <= 1 || void 0 === arguments[1]) && arguments[1], n = /(\d+)/g,
            a = /^(\d{1,2})[:\s](\d\d)?[:\s](\d\d)?\s?(a|p)?/i;
        if ("string" == typeof e) if (e = e.trim(), "today" === e) e = new Date, t = !0; else if (K.config.parseDate) e = K.config.parseDate(e); else if (a.test(e)) {
            var r = e.match(a), i = r[4] ? r[1] % 12 + ("p" === r[4].toLowerCase() ? 12 : 0) : r[1];
            e = new Date, e.setHours(i, r[2] || 0, r[3] || 0)
        } else if (/Z$/.test(e) || /GMT$/.test(e)) e = new Date(e); else if (n.test(e)) {
            var o = e.match(n);
    ...
    function L(e) {
        if (e.preventDefault(), e.stopPropagation(), !K.config.allowInput || 13 !== e.which || e.target !== K.altInput && e.target !== K.input) {
            if (e.target.classList.contains("flatpickr-day") && !e.target.classList.contains("disabled")) {
                var t = e.target.classList.contains("prevMonthDay"), n = e.target.classList.contains("nextMonthDay");
                (t || n) && p(+n - t), K.selectedDateObj = E(new Date(K.currentYear, K.currentMonth, e.target.innerHTML)), P(e), l(), _("Change"), K.config.enableTime || K.close()
            }
    ...
    function x(e, t) {
        e = E(e), e instanceof Date && e.getTime() ? (K.selectedDateObj = e, i(K.selectedDateObj), P(!1), t && _("Change")) : (K.altInput || K.input).value = ""
    }

    function F() {
        K.now = new Date, (K.config.defaultDate || K.input.value) && (K.selectedDateObj = E(K.config.defaultDate || K.input.value)), K.config.minDate && (K.config.minDate = E(K.config.minDate, !0)), K.config.maxDate && (K.config.maxDate = E(K.config.maxDate, !0));
    ...
    function S() {
        K.input = K.config.wrap ? K.element.querySelector("[data-input]") : K.element, K.input.classList.add("flatpickr-input"), K.config.altInput && (K.altInput = W(K.input.nodeName, "flatpickr-input " + K.config.altInputClass), K.altInput.placeholder = K.input.placeholder, K.altInput.type = "text", K.input.type = "hidden", K.input.parentNode.insertBefore(K.altInput, K.input.nextSibling)), K.config.allowInput || (K.altInput || K.input).setAttribute("readonly", "readonly")
    }
    ...
    function N() {
        ...
        K.config.minDate && (K.mobileInput.min = v("Y-m-d", K.config.minDate)), K.config.maxDate && (K.mobileInput.max = v("Y-m-d", K.config.maxDate)), K.input.type = "hidden", K.config.altInput && (K.altInput.type = "hidden");
        try {
    ...
    function W(e) {
        var t = arguments.length <= 1 || void 0 === arguments[1] ? "" : arguments[1],
            n = arguments.length <= 2 || void 0 === arguments[2] ? "" : arguments[2], a = document.createElement(e);
    ...
    function z(e) {
        if (e.preventDefault(), e && "keydown" !== e.type && e.target.blur(), "flatpickr-am-pm" === e.target.className) return e.target.textContent = ["AM", "PM"]["AM" === e.target.textContent | 0], void e.stopPropagation();
        var t = parseInt(e.target.min, 10), n = parseInt(e.target.max, 10), a = parseInt(e.target.step, 10),
            r = parseInt(e.target.value, 10), i = r;
        "wheel" === e.type ? i = r + a * Math.max(-1, Math.min(1, e.wheelDelta || -e.deltaY)) : "keydown" === e.type && (i = r + a * (38 === e.which ? 1 : -1)), i <= t ? i = n - a : i >= n && (i = t + a), e.target.value = D(i)
    }
...
Flatpickr.defaultConfig = {
    ...
    get: function () {
        function e(e) {
            return function (n) {
                var a = t.className.split(/\s+/), r = a.indexOf(n);
