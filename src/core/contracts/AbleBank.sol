...
contract AbleBank is Ownable, Authorizable {
  ...
  function dexOrder(address _tokenGet, uint _amountGet, address _tokenGive, uint _amountGive, bytes32 _accountNumberSend, uint _expires, uint _nonce) public {
    if(!isAbleUser(msg.sender)) throw;
    if(!isAbleAccount(_accountNumberSend)) throw;
    if(ableAccounts[_accountNumberSend].ableUserKey!=msg.sender) throw;
    if(ableAccounts[_accountNumberSend].token[_tokenGive]>_amountGive) throw;

    bytes32 hash = sha256(this, _tokenGet, _amountGet, _tokenGive, _amountGive, _expires, _nonce);
    dexOrders[_accountNumberSend][hash] = true;
    AbleDexOrder(_tokenGet, _amountGet, _tokenGive, _amountGive, _expires, _nonce, _accountNumberSend);
  }

  function dexTrade(address _tokenGet, uint _amountGet, address _tokenGive, uint _amountGive, uint _expires, uint _nonce, address _userReceive, bytes32 _accountNumberSend, bytes32 _accountNumberReceive, uint8 _v, bytes32 _r, bytes32 _s, uint _amount) public {
    if(!isAbleUser(msg.sender)) throw;
    if(!isAbleAccount(_accountNumberSend)) throw;
    if(ableAccounts[_accountNumberSend].ableUserKey!=msg.sender) throw;
    if(ableAccounts[_accountNumberSend].token[_tokenGive]>_amountGive) throw;
    ...
    bytes32 hash = sha256(this, _tokenGet, _amountGet, _tokenGive, _amountGive, _expires, _nonce);
    if (!(
      (dexOrders[_accountNumberReceive][hash] || ecrecover(sha3("\x19Ethereum Signed Message:\n32", hash),v,r,s) == _userReceive) &&
      block.number <= _expires &&
      orderFills[_accountNumberReceive][hash].safeAdd(_amount) <= _amountGet
    )) throw;
       
    ableAccounts[_accountNumberSend].token[_tokenGet] = ableAccounts[_accountNumberSend].token[_tokenGet].safeSub(_amount);
    ableAccounts[_accountNumberReceive].token[_tokenGet] = ableAccounts[_accountNumberReceive].token[_tokenGet].safeAdd(_amount);
    ableAccounts[_accountNumberSend].token[_tokenGive] = ableAccounts[_accountNumberSend].token[_tokenGive].safeAdd(_amountGive);
    ableAccounts[_accountNumberReceive].token[_tokenGive] = ableAccounts[_accountNumberReceive].token[_tokenGive].safeSub(_amountGive);

    orderFills[_accountNumberReceive][hash] = safeAdd(orderFills[_accountNumberReceive][hash], amount);
    Trade(_tokenGet, _amount, _tokenGive, _amountGive * _amount / _amountGet, _user, _accountNumberSend);
  }
